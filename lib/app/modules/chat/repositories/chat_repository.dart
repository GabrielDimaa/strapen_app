...
class ChatRepository implements IChatRepository {
  ...
  ParseObject toParseObject(ChatModel model) {
    ParseObject parseObject = ParseObject(className())
      ...
      ..set<ParseObject>(CHAT_LIVE_COLUMN, ParseObject(LiveRepository().className())..set(LIVE_ID_COLUMN, model.live!.id));
  ...
  ChatModel toModel(ParseObject e) {
    return ChatModel(
      ...
      LiveRepository().toModel(e.get(CHAT_LIVE_COLUMN)),
    );
  }
  ...
  QueryBuilder<ParseObject> queryBuilder(LiveModel liveModel) {
    return QueryBuilder(ParseObject(className()))
      ..whereEqualTo(
        CHAT_LIVE_COLUMN,
        (ParseObject(LiveRepository().className())..set(CHAT_ID_COLUMN, liveModel.id)).toPointer(),
      )
